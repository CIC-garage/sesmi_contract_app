...
sap.ui.define([
	...
	return BaseController.extend("com.cicre.po.controller.Object", {
		...
		_onObjectMatched: function (oEvent) {
			...
			mModel.read("/ContractPOHeaderSet('" + sObjectId + "')", {
				urlParameters: {
					"$expand": "POHeaderToBoqItem,ContractPONoteNav,POHeaderToPOItem,ContractPOSrvItemSet,AttachmentSet,HeadToHeadVariationNav/OrderItemsSet,POHeaderToPOGetbuildingNav,ReleaseerrorSet,PermissionNav,WFContractInboxNAV,POPurchaseRequisitionTreeNav"
				},
				success: function (oData) {
					...
					oData.ContractPONoteSet = oData.ContractPONoteNav;
					...
					$.each(oData.myPurchaseRequisition, function (W, BuildingSet) {

						oData.myPurchaseRequisition[W].DelInd = "";
						oData.myPurchaseRequisition[W].ChangeIndicator = "";
						oData.myPurchaseRequisition[W].Group = oData.myPurchaseRequisition[W].SubBoq;
						oData.myPurchaseRequisition[W].Model = oData.myPurchaseRequisition[W].Boq;
						oData.myPurchaseRequisition[W].BuildingNo = oData.myPurchaseRequisition[W].BuildingNo;
						oData.myPurchaseRequisition[W].IdText = oData.myPurchaseRequisition[W].ModelDesc;
						oData.myPurchaseRequisition[W].WbsCode = oData.myPurchaseRequisition[W].Wbs;
						oData.myPurchaseRequisition[W].PrNo = oData.myPurchaseRequisition[W].BuildingNo;
						oData.myPurchaseRequisition[W].ModelType = oData.myPurchaseRequisition[W].Type
						//oData.myPurchaseRequisition[W].Network = Network
						//oData.myPurchaseRequisition[W].ActivityNumber = ActivityNumber

						WbslistNo = parseFloat(WbslistNo) + 1;
						if (oData.myPurchaseRequisition[W].ContractualIndicator === "O") { oData.myPurchaseRequisition[W].sortNum = 1 }
						if (oData.myPurchaseRequisition[W].ContractualIndicator === "V") { oData.myPurchaseRequisition[W].sortNum = 2 }
						if (oData.myPurchaseRequisition[W].ContractualIndicator === "A") { oData.myPurchaseRequisition[W].sortNum = 3 }
						$.each(that.HeadToHeadVariationG, function (j, V) {
							if (oData.myPurchaseRequisition[W].Txt === V.OrderType + "-" + V.UserOrder) {
								oData.myPurchaseRequisition[W].OrderNo = V.OrderNo;
							}
					...
					oData.PurchaseRequisitionNO = oData.myPurchaseRequisition.length;
					...
					oData.TotalvariationMonths = 0;
					oData.TotalvariationDays = 0;
					oData.TotalContractDurationMonths = 0;
					oData.TotalContractDurationDays = 0;
					oData.HeadToHeadVariation.map(function (oVariationOrder) {
						oVariationOrder.OrderItemsSrvSet = [];
						oVariationOrder.DaysNo = parseInt(oVariationOrder.DaysNo);
						oVariationOrder.MonthNo = parseInt(oVariationOrder.MonthNo);
						oData.TotalvariationMonths = parseInt(oData.TotalvariationMonths) + parseInt(oVariationOrder.MonthNo);
						oData.TotalvariationDays = parseInt(oData.TotalvariationDays) + parseInt(oVariationOrder.DaysNo);
					...
					oData.TotalContractDurationMonths = parseInt(oData.TotalvariationMonths) + parseInt(oData.ContDurationMonth);
					oData.TotalContractDurationDays = parseInt(oData.TotalvariationDays) + parseInt(oData.ContDurationDays);
					oData.ContDurationMonth = parseInt(oData.ContDurationMonth);
					oData.ContDurationDays = parseInt(oData.ContDurationDays);

					//this.calculateDates(oData.ValidFrom,TotalContractDurationMonths,TotalContractDurationDays);
					oData.ValidTo = that.calculateDates(oData.ValidFrom, oData.TotalContractDurationMonths, oData.TotalContractDurationDays)
					...
					//that.getStartDate(CommencementDate,DateofApplying,SignoffDate);
					...
					if (oData.CreationType === "E") {
						...
						var AsbuildService = [];
						var ContractPOSrvItemSet = that.POHeaderToPOSrv;
						...
						oData.BuildingConsulidatedItemSetfilter = or_V_A;
						...
						srvlisttt = that.Consoliatedserviceslistobject(that.POHeaderToPOSrv);
						//sort for 
						...
						setTimeout(function () {
							...
							oData.PRItemSet  = that._handleTreePR(oData.POPurchaseRequisitionTreeNav.results);
						...
						//	$.each(consolidate, function (c, cons) {
								$.each(BoqItemSet, function (i, boq) {
									if (cons.boq === boq.Data) {
									$.each(boq.Categories, function (j, sub) {

										$.each(sub.Categories, function (l, cat) {
											$.each(cat.Categories, function (k, srv) {
												findservice = consolidate.find(x => x.Data === srv.Data);
												if (findservice.Data === srv.Data && findservice.Txt === srv.Txt) {
													BoqItemSet[i].Categories[j].Categories[l].Categories[k].ProvisionRate = (parseFloat(findservice.ProvisionRate));
													BoqItemSet[i].Categories[j].Categories[l].Categories[k].PriceUnit = (parseFloat(findservice.PriceUnit) * parseFloat(BoqItemSet[i].Categories[j].Categories[l].Categories[k].Qty));
													if (srv.Eancat !== "C" && srv.Eancat !== "U") { BoqItemSet[i].PriceUnit = (parseFloat(BoqItemSet[i].PriceUnit) + (parseFloat(findservice.PriceUnit) * parseFloat(BoqItemSet[i].Categories[j].Categories[l].Categories[k].Qty))).toString(); }
													if (srv.Eancat !== "C" && srv.Eancat !== "U") { BoqItemSet[i].Categories[j].PriceUnit = (parseFloat(BoqItemSet[i].Categories[j].PriceUnit) + (parseFloat(findservice.PriceUnit) * parseFloat(BoqItemSet[i].Categories[j].Categories[l].Categories[k].Qty))).toString(); }
													if (srv.Eancat !== "C" && srv.Eancat !== "U") { BoqItemSet[i].Categories[j].Categories[l].PriceUnit = (parseFloat(BoqItemSet[i].Categories[j].Categories[l].PriceUnit) + (parseFloat(findservice.PriceUnit) * parseFloat(BoqItemSet[i].Categories[j].Categories[l].Categories[k].Qty))).toString(); }
